...
let numUlList = 3; //Number ul list in country list
function createCountryList() {
  const numCountries = country_list.length;
  let i = 0,
    ulListId;
  country_list.forEach((country, index) => {
    if (index % Math.ceil(numCountries / numUlList) == 0) {
      ulListId = `list-${i}`;
...
// Search Filter
// Triggers only when user type something in input field
searchInput.addEventListener("input", () => {
  let input = searchInput.value.toUpperCase();

  country_list.forEach((country) => {
    if (country.name.toUpperCase().startsWith(input)) {
